...
public class CloneFactory {
    ...
    public static Product copy(Product target, ProductAllPo source){
        ...
        target.setOnSaleList(source.getOnSaleList().stream()
                .map(o->CloneFactory.copy(new OnSale(), o)).collect(Collectors.toList()));
    ...
    public static Product copy(Product target, ProductJoinPo source) {
        target = Product.builder()
                ...
                .creator(User.builder().id(source.getCreatorId()).name(source.getCreatorName()).build())
                .modifier(User.builder().id(source.getModifierId()).name(source.getModifierName()).build())
        ...
        // 复制 OnSaleList
        target.setOnSaleList(source.getOnSaleList().stream()
                .map(o -> CloneFactory.copy(new OnSale(), o))
                .collect(Collectors.toList()));

        // 复制 OtherProduct，过滤掉与当前 Product ID 相同的项
        Long id = target.getId();
        target.setOtherProduct(source.getOtherProduct().stream()
                .filter(o -> !o.getId().equals(id))
                .map(o -> CloneFactory.copy(new Product(), o))
                .collect(Collectors.toList()));
